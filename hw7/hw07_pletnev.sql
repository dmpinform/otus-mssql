...
FROM OPENROWSET
(BULK '/var/opt/mssql/backups/StockItems.xml', 
 SINGLE_CLOB)
as data 
...
SELECT StockItemID, 
	   ...
	   (select STRING_AGG(cast(tags.value as nvarchar(max)), ', ') FROM Warehouse.StockItems
		CROSS APPLY OPENJSON(CustomFields, '$.Tags') tags 
		where StockItemID = stock.StockItemID
		group by StockItemID, StockItemName) as tag_list		
